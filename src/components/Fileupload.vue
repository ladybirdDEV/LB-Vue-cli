<template>
  <div :class="className">
    <label :for="name">
      <input type="file" :name="name" :id="id" :accept="accept" @click="fileInputClick" @change="fileInputChange" :multiple="multiple">
      <slot></slot>
    </label>
    <button type="button" @click="fileUpload">{{ buttonText }}</button>
  </div>
</template>
<script>
export default {
